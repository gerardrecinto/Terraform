...
The S3 lifecycle module alone delivered **$26.28M/year in cost savings** by tiering ~9 PB of Axiom log data to Glacier Deep Archive across Oregon, Mumbai, and Frankfurt.
...
└── workflows/
    └── deploy_apigee_proxies/ DeviceCloud, SoftwareHub, ModelHub proxies with token auth + path routing
```
...
Tiered storage that saved $26.28M/year on 9 PB of Axiom log data:

```hcl
module "axiom_logs" {
  source = "./AWS/modules/s3_lifecycle"

  bucket_name                = "axiom-logs-prod"
  environment                = "prod"
...
Reduced 79 fragmented proxies to 17 (78%), cutting Apigee latency from 18,094ms to 540ms (97%).
...
DeviceCloud, SoftwareHub, ModelHub proxies in production:

| Proxy | Base Path | Routes |
|---|---|---|
| DeviceCloud | `/devicecloud/v2` | `/devices`, `/ssh`, `/workspaces`, `/builds` |
| SoftwareHub | `/softwarehub/v2` | `/packages`, `/download`, `/catalog`, `/releases` |
| ModelHub | `/modelhub/v2` | `/models`, `/inference`, `/benchmarks`, `/compile`, `/profile` |
