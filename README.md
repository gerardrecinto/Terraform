...
Cross-account PrivateLink for the DeviceService SSH/ADB Gateway and device streaming. NLB-backed endpoint service on the provider side; VPC endpoint with Route53 alias on the consumer side. No VPC peering, no public internet exposure.

```hcl
module "ssh_gateway_privatelink" {
  source = "./AWS/modules/privatelink"

  service_name       = "device_service-ssh-gateway"
...
module "telemetry_logs" {
  ...
  bucket_name                = "examplecorp-telemetry-logs-prod"
  environment                = "prod"
...
module "grafana_alerts" {
  ...
  alb_names          = ["device_service", "inference_service"]
...
module "nginx" {
  ...
  cluster_name = "device_service-prod"
  environment  = "prod"
  vpc_id       = var.vpc_id
  use_nlb      = true
